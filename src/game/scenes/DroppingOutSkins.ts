...
export class DroppingOutSkins extends Scene {
    ...
    public init() {
        ...
        const bushes = new PIXI.Sprite(app.getTexture("bushes2"));
        bushes.position.x = 14;
        bushes.position.y = 352;
        this.addChild(bushes);

        const tree = new PIXI.Sprite(app.getTexture("tree2"));
        tree.position.x = -31;
        tree.position.y = 226;
        this.addChild(tree);
        ...
        const skinsMachine = new PIXI.Sprite(app.getTexture("skinsMachine"));
        skinsMachine.position.x = 15;
        skinsMachine.position.y = 20;
        this.addChild(skinsMachine);

        const titleTxt = new PIXI.Text("DroppingOutSkins");
    ...
    public runScene(oldScene: Scene) {
        console.log("DroppingOutSkins show");
    }
}
